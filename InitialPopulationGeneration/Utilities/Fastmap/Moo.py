...
class Moo(BinaryTree):
    ...
    def divide(i,abort=False,minnie=30):
        ...
        if not i.abort and n >= minnie :
            ...
            if i.west != i.east:
                ...
                westLoss = loss(weightedWest, weightedEast)
                eastLoss = loss(weightedEast, weightedWest)
                EPSILON = 1.0
    ...
    def binaryChop(i, rows, cut, delta, min_n, lastcut=None, distance_matrix=None):
        ...
        if distance_matrix is not None:
            left_dm, right_dm = i.distance_matrix_transformation(distance_matrix, cut)
        else:
            z = len(i.problem.decisions)
            distance_matrix = i.calc_distance_matrix([l.cells[:z] for l in rows])
            left_dm, right_dm = i.distance_matrix_transformation(distance_matrix, cut)


        #get spreads (VARIANCE) of each side
        leftSpread = spacing(left_dm) #var([l.x for l in left])
        rightSpread = spacing(right_dm) #var([r.x for r in right])
        delta = abs(leftSpread - rightSpread)
        #print delta, cut
        #recurse
        lhscut,lhsdelta = i.binaryChop(rows, cut/2, delta, min_n, cut, distance_matrix=distance_matrix)
        rhscut,rhsdelta = i.binaryChop(rows, cut + (lastcut - cut)/2, delta, min_n, cut, distance_matrix=distance_matrix)
